...
import (
	...
	"github.com/coinbase/chainsformer/internal/utils/retry"
	"github.com/coinbase/chainsformer/internal/utils/testutil"
	"github.com/coinbase/chainsformer/internal/utils/timesource"
)
...
func TestInstrument(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.hasError && test.errorValue > 0 {
				errorCounter := snapshot.Counters()[fmt.Sprintf("foo.bar+result_type=error,status=%s", status.Code(err).String())]
				require.NotNil(errorCounter)
				require.Equal(test.errorValue, errorCounter.Value())
				require.Equal(map[string]string{
					resultType: resultTypeError,
					statusTag:  status.Code(err).String(),
				}, errorCounter.Tags())
			}
